...
{
    public partial class Form_VerComprasDia : Form
    {
        public Form_VerComprasDia()
        {
            InitializeComponent();
        }
        public static ArrayList Aboleta = new ArrayList();
        private void Form_VerComprasDia_Load(object sender, EventArgs e)
        {
            Aboleta.Clear();
            Cls_N_ReportesCompras objN = new Cls_N_ReportesCompras();
        ...
        {
            Cls_N_ReportesCompras.acumulador = 0;
            ...
            foreach(Cls_E_Boleta item in temp)
            {
               
                    Cls_N_ReportesCompras.acumulador = Cls_N_ReportesCompras.acumulador + item.Total;
                
            }
            txtTotal.Text = Cls_N_ReportesCompras.acumulador.ToString();
            mtdPintarCeldas();
        }
        ...
        private void BtnVerRangoFecha_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow fila in dgBoleta.Rows)
            {
                ...
                if (fecha >= fechainicio && fecha <= fechafin && fila.Cells[8].Value.ToString() != "0")
                {
                    ...
                    objEBoleta.Estado = fila.Cells[8].Value.ToString();
                    temp.Add(objEBoleta);
                    Cls_N_ReportesCompras.acumulador = Cls_N_ReportesCompras.acumulador + float.Parse(fila.Cells[7].Value.ToString());
                }
            }
            dgConsultaBoleta.DataSource = null;
            dgConsultaBoleta.DataSource = temp;
        ...
        private void DgConsultaBoleta_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            Cls_N_ReportesCompras objN = new Cls_N_ReportesCompras();
            dgBoleta.DataSource = null;
