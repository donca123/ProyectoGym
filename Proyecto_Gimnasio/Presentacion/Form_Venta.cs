...
namespace Proyecto_Gimnasio.Presentacion
{
    public partial class Form_Venta : Form
    {
        ...
        private void Form_Venta_Load(object sender, EventArgs e)
        {
            ...
            clientes = objN.MtdLIstarClientes();
            foreach (DataRow fila in clientes.Rows)
            {
                if (fila[2].ToString() == "1")
                {
                    cbCliente.Items.Add(fila[0].ToString().ToUpper());
                }
            }


        }
        public static ArrayList Aproductos = new ArrayList();
        public static ArrayList AcomboProductos = new ArrayList();
        public static ArrayList Adetalleventa = new ArrayList();
        public static ArrayList Aventa = new ArrayList();
        ...
        private void TxtProveedor_TextChanged(object sender, EventArgs e)
        {
            ...
            cbProductos.DataSource = AcomboProductos;
        }
            ...
            if (cbProductos.SelectedIndex != -1)
            {
                foreach (DataRow fila in productos.Rows)
                {
                    if (fila[1].ToString() == cbProductos.SelectedItem.ToString())
                    {
                        ...
                        txtPrecioUnitario.Text = fila[2].ToString();
                    }
                }
            }
        }

        private bool MtdVerificarStock(string cantidad)
        {
            foreach (DataRow fila in productos.Rows)
            {
                if (fila[0].ToString() == lblProducto.Text)
                {
                    if (int.Parse(cantidad) > int.Parse(fila[4].ToString()))
                    {
                        return false;
                    }
                }
        ...
        private void PicAgregar_Click(object sender, EventArgs e)
        {
            ...
            subtotal = 0;
            if (cbProductos.SelectedIndex == -1 || cbCliente.SelectedIndex == -1 || txtPrecioUnitario.Text == "" || txtCantidadProducto.Text == "")
            {
                MessageBox.Show("Seleccione campos");
                return;
            }
            if (MtdVerificarStock(txtCantidadProducto.Text))
            {
            ...
            foreach (DataGridViewRow fila in dgvCompra.Rows)
            {
                if (lblProducto.Text == fila.Cells[0].Value.ToString())
                {
                    ...
                    if (MtdVerificarStock(cantidad.ToString()) == true)
                    {
        ...
        private void PicGuardar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (MessageBox.Show("¿Registrar Venta?", "Alerta", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
                {
                    ...
                    objEBoleta.Estado = "1";
                    objEBoleta.Cliente = cbCliente.SelectedItem.ToString();
                    ...
                    if (comprobar == true && comprobar2 == true)
                    {
                        ...
                        foreach (DataGridViewRow fila in dgvCompra.Rows)
                        {
                            ...
                            objN.MtdDecrementarProductos(objE);
                        }
        ...
        private void CbCliente_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbCliente.SelectedIndex != -1)
            {
                foreach (DataRow fila in clientes.Rows)
                {
                    if (fila[0].ToString() == cbCliente.SelectedItem.ToString())
                    {
                        lblCliente.Text = fila[1].ToString().ToUpper();
                    }
                }
            }
        }
